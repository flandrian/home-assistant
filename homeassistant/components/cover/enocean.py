...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    base_id_offset = config.get('base_id_offset')
    channel = config.get('channel')
    
    add_entities([EnOceanCover(dev_id, name, base_id_offset, channel)])


class EnOceanCover(EnOceanDevice, CoverDevice):

    def __init__(self, dev_id, name, base_id_offset, channel):
        EnOceanDevice.__init__(self, dev_id)
        self._name = name
        self._transmitter_id = self.get_base_id(base_id_offset)
        self._channel = int(channel)
        self._position = 0
        
    def handle_packet(self, radio_packet):
    ...
    def _send_move_packet(self, position):
        self.create_and_send_packet(rorg=0xD2, rorg_func=0x05, rorg_type=0x01, command=1,
                                  sender=self._transmitter_id,
                                  CMD=1,
                                  POS=position,
                                  ANG=position,
                                  REPO=0,
    ...
    def stop_cover(self):
        self.create_and_send_packet(rorg=0xD2, rorg_func=0x05, rorg_type=0x01, command=2,
                                  sender=self._transmitter_id,
                                  CMD=2,
                                  CHN=self._channel)
